...
import { CardInformation } from "../infrormation";
...
class GameStore {
  ...
  @observable
  displayTurn: displayTurn = { teamName: "bora", borderColor: "white", avatar: "" };
  ...
  activateSpinAnimation = (numberSpinned: number) => { 
    runInAction (() =>  { 
      ...
      this.actionPicker = {actionType: "chance", numberSpinned: numberSpinned};
      this.spinning = true;
    })
  }
  @action 
  ...
  @action 
  notEnoughScores = () => { 
    this.shopInformation = {inform: "У вас недостаточно очков для покупки этой карты", type: null, amount: null}
  }
  @action 
  informPlayerPurchase = (data: CardInformation) => {
    this.shopInformation = {inform: "Вы приобрели", type: data.type, amount: data.amount}
  }
